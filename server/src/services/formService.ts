...
export async function salvarFormularioCompleto(dadosForm: any, userEmail: string | null) {
  ...
  const requests = (dadosForm.questoes || []).map((questao: any, index: number) => {
...
export async function salvarPergunta(form: NewQuestFormSaved) {
  const repo = AppDataSource.getRepository(Pergunta);
  const tipo = await AppDataSource.getRepository(Tipo_Pergunta).findOneBy({ Descricao: form.tipo });
